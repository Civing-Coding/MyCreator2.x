...
@ccclass
export default class NumberType extends cc.Component {
    ...
    private excuteType(n: number, afterPointfixed: number = 0) {
        ...
        this.schedule(() => {
            ...
            this.label.string = parseFloat(<any>cc.misc.lerp(this._curNum, n, index)).toFixed(afterPointfixed).toString();
            if (index == 1) {
                this._curNum = parseFloat(this.label.string);
                this._run = false;
