...
const EVENT_TYPE = cc.Enum({
    EVENT_MANAGER: 0,
    NODE: 1,
    COMPONENT: 2,
})

const EVENT_COMPONENT_TYPE = cc.Enum({
    ON_LOAD: 0,
    START: 1,
    ON_ENABLE: 2,
    UPDATE: 3,
...
@ccclass
export default class EventTools extends cc.Component {
    ...
    private _obj: cc.Node = null;

    onLoad() {
        this._obj = this.target || this.node;

        this.eventType == EVENT_TYPE.COMPONENT && this.componentType == EVENT_COMPONENT_TYPE.ON_LOAD && this.excute();
        this.eventType == EVENT_TYPE.NODE && this._obj.on(cc.Node.EventType[EVENT_NODE_TYPE[this.nodeType]], this.excute, this);
    ...
    onDisable() {
        ...
        this.eventType == EVENT_TYPE.NODE && this._obj.off(cc.Node.EventType[EVENT_NODE_TYPE[this.eventType]], this.excute, this);
        if (this.eventType == EVENT_TYPE.EVENT_MANAGER) {
    ...
    excute(args?: any) {
        cc.Component.EventHandler.emitEvents(this.call_functions, args);
    }
}
